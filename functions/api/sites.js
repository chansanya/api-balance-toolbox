...
export async function onRequestGet({ env }) {
  const { results } = await env.DB
    .prepare('SELECT id, name, url, note, tags, created_at FROM sites ORDER BY id DESC')
    .all();
  return ok(results.map((r) => ({ ...r, tags: parseTags(r.tags) })));
}

// POST /api/sites  {name, url, note?, tags?:[]}  新增公益站点(需登录,中间件已校验口令)
export async function onRequestPost({ request, env }) {
  ...
  const res = await env.DB
    .prepare('INSERT INTO sites (name, url, note, tags) VALUES (?, ?, ?, ?)')
    .bind(name, url, note, JSON.stringify(tags))
    .run();
  return ok({ id: res.meta?.last_row_id, name, url, note, tags }, '添加成功');
}
